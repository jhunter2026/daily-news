...
- Required env vars: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `GEMINI_API_KEY`. Optional: `BACKFILL_SECRET`.
...
- Neither the cron route nor the backfill route requires auth unless `BACKFILL_SECRET` is set (which only gates backfill) — anyone with the cron URL can trigger a full scoring + email run. Both now run with the service-role key, so this matters more than it used to: an open cron URL can fully read/write the table, not just insert.
